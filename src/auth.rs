...
use std::{convert::Infallible, fmt::{self, Formatter}};
use warp::{
    Filter, Rejection,
...
// pub fn with_auth(role: Role) -> impl Filter<Extract = One<HeaderMap>, Error = Infallible> + Copy {
    // impl Filter<Extract = One<HeaderMap>, Error = Infallible> + Copy
    headers_cloned()
